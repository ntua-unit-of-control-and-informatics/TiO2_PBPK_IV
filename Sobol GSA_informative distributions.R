...
# the output matrix
n<-1000 # Number of samples for each parameter
...
S_1 <- matrix(NA, nrow = length(init_prior), ncol = 12)
S_total <- S_1
rownames(S_1) <- param_names <- c("x_lu","x_spl","x_li", "x_ki", "x_ht", "x_br", "x_ut", "x_skel", "x_st", "P_lu",  "P_spl", 
                                  ...
                                  "k_de")
colnames(S_1) <- c("Lungs", "Spleen", "Liver", "Kidneys", "Heart", "Brain", "Uterus", "Skeleton", "Soft tissue", "Blood", "Feces", "Urine")
...
for (data in observed_data) {
  xy_values <- data[,1:2]
  AUC_integration <- sintegral(xy_values[,1], xy_values[,2], n.pts = 256)
  AUC_obs[counter] <- as.numeric(AUC_integration["value"]) 
  counter <- counter+1
}
...
Volumes <- c(params_list$Vtis_lu, params_list$Vtis_spl, params_list$Vtis_li, params_list$Vtis_ki, params_list$Vtis_ht, params_list$Vtis_br, params_list$Vtis_ut,
             params_list$Vtis_skel, params_list$Vtis_st, params_list$V_blood,
             V_feces, V_urine)
w <- AUC_obs/Volumes # Calculate the scaling factors

###Calculate weighted first and total order indices
first_weighted <-rep(0,length(init_prior))
total_weighted <-first_weighted
for (i in 1:length(init_prior)) {
  first_weighted[i] <- sum(w*S_1[i,])/sum(w)
  total_weighted[i] <- sum(w*S_total[i,])/sum(w)
}

param_names <- c("x_lu","x_spl","x_li", "x_ki", "x_ht", "x_br", "x_ut", "x_skel", "x_st", "P_lu",  "P_spl", 
                 "P_li","P_ki", "P_ht", "P_br", "P_ut", "P_skel",  "P_st" , "uptake_spl", "uptake_ki", "uptake_ht", "uptake_br",
                 "uptake_ut", "uptake_skel", "uptake_st", "uptake_blood","uptake_lu", "uptake_li", "Pup_max", "CLE_f", "CLE_u",
                 "k_de")
...
Sobol_plot <- ggplot(sobol_data, aes(param_names, value, fill=variable))+
...
setwd("__")
comp_names <- c( "Lungs", "Spleen", "Liver", "Kidneys", "Heart", "Brain", "Uterus", "Skeleton", "Soft_tissue",  "Blood", "Feces", "Urine")
for (i in 1:length(comp_names)) {
